...
const AuthDetails = () => {
  return (
    ...
    <Card style={{maxWidth: 500, height: '420px', margin: '10px', background: 'linear-gradient(to right, #FFF5FF, #FFE5FF)', boxShadow: 'rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px, rgba(10, 37, 64, 0.35) 0px -2px 6px 0px inset'}}>
        ...
        <Grid container spacing={2} style={{ display: 'flex', flexDirection: 'column'}}>
          {items.map((item, index) => (
            <Grid item xs={12} sm={6} md={3} key={index} style={{ display: 'flex', flexDirection: 'row'}}>
              ...
              <Typography variant="body1" style={{ color: '#330033', fontWeight: 'bold', fontSize: '15px', width: '300px', marginTop: '10px', WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale' }}>{item.text1}</Typography>
              <Typography variant="body2" style={{color: 'black', width: '300px', fontSize: '12px'}}>{item.text2}</Typography>
              </div>
            </Grid>
          ))}
        </Grid>
