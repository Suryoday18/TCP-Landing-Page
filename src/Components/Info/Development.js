        ...
        <List>
          {IMAGES.map((image, index) => (
            <ListItem key={index} className={classes.listItem}>
              ...
              <Typography variant="body1" style={{marginLeft: '20px', color: '#1F4A7C', marginTop: '20px'}}>{image.description}</Typography>
            </ListItem>
          ))}
        </List>
      </CardContent>
      <Button onClick={handleReadMoreClick} style={{width: '120px', height: '40px', margin: '20px',   padding: '5px', borderColor: '#1F4A7C', color: '#1F4A7C', border: '2px solid #1F4A7C', backgroundColor: 'transparent', }}>Read More +</Button>
    </Card>
  );
}
